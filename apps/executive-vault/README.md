...
**v6** is a **visual/interaction alignment pass against `apps/svegip`** (see `docs/architecture/executive-vault-ui-alignment.md` for the full reuse assessment): the sidebar, topbar, login screen, tables, chips and page headers now read as the same design family as SVEGIP — same structural conventions (grouped sidebar nav with an active-state accent bar, sticky blurred topbar, pill badges, an "eyebrow" label above page titles, an 820px sidebar-collapse breakpoint with a backdrop overlay, a two-panel login layout) — while staying visually and functionally distinct: Executive Vault keeps its own navy/gold/serif identity (never SVEGIP's purple/orange), its own information architecture, and no SVEGIP business modules. No screen, route, data model, or authentication logic changed.

**v7** is a focused usability pass: Document Vault, Archive, and Projects/Matters now render a mobile-only record-card list (Identity/Context/State — never every desktop column) below 600px instead of forcing a horizontally-scrolled table, adapting SVEGIP's mobile-card principle; Actions & Follow-Up rows reflow instead of truncating at narrow widths; Search Results and Clients & Engagements were already card/list-based and needed no change. The Meeting Brief's pre-meeting workflow is now formalised as **Prepare Meeting** — the same drawer, an explicit action, and a consolidated read-only view (latest relevant documents, documents for review, outstanding actions, waiting on, pending decisions, the previous related meeting) assembled entirely from existing records, with an explicit way back to the normal brief. Management Progress is untouched. No route, data model, or authentication logic changed.

**v8** is a visual-depth/interaction refinement pass, still within the navy/gold identity: a subtle sidebar gradient and a stronger, glowing active-nav state; a secondary signal-blue/cyan accent layered on top of gold for interactivity and "live now" cues (never replacing gold as the primary brand colour); hover-elevation and shadow depth on clickable cards, buttons and mobile record cards; a restrained pulsing `.live-dot` used only when a meeting is genuinely happening right now; top-accent bars that give each Executive Home card a distinct identity; severity-coloured Attention Required icons; a richer stat-pill summary strip; and small polished micro-interactions (drawer content fade-in, close-button rotate, popover pop-in, a one-time login-card entrance). Management Progress, the information architecture, the data model, and the authentication foundation are all unchanged — this is CSS/markup polish only.
...
**Command Centre:** Executive Home (compact header/greeting, summary strip, Today, Attention Required + Waiting On, Recently Modified + Requires Review or Decision) · My Day · This Week · Meeting Brief (drawer, opened from any calendar event), with a **Prepare Meeting** action on any upcoming meeting that has a Client set — the same drawer expands into a consolidated, structured-data-only preparation state (latest relevant documents, documents for review, outstanding actions, waiting on, pending decisions, the previous related meeting), with an explicit "← Back to Meeting Brief" to return · Projects / Matters (cross-client register) · a header Notifications/Attention popover · Outlook Calendar (placeholder).
