...
--
-- status is the smallest model that distinguishes requested / completed /
-- failed-and-retryable (PR brief item 15) — deliberately NOT a generic
-- job-processing table: no queue semantics, no worker-lease columns, no
-- priority/backoff schedule. A "processing" state was assessed and
-- rejected: this codebase's transaction-scoped processor either commits
-- the whole disable+audit+completion in one go or rolls all of it back
-- (see identityDeactivationProcessor.ts), so there is never an
-- observable, durable "in progress" state worth persisting.
--
