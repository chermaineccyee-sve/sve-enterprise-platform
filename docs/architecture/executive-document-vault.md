...
| `managementAttentionLevel` | enum or null: `Decision Required` / `For Review` / `Direction Required` / `Approval Required` | `null` | Drives the dedicated Management Attention section (§16.3) — **null means routine, not shown there**, exactly per the brief's "ONLY matters requiring management action." **Usage decision, 21 Sep 2026: standardise on `Decision Required` wherever management is actually required to decide something** (including direction-setting decisions) — `managementAttentionNote` carries the nuance ("management direction required on …"), not a separate level, so the badge vocabulary a reader learns once stays consistent. `Direction Required`/`For Review`/`Approval Required` remain available in the vocabulary for a case that is genuinely not a decision (e.g. a pure sign-off with no direction implied) — the decision is about which one to reach for by default, not removing the others. |
...
**Command Centre visibility indicators (added 21 Sep 2026):** so the account holder never has to open Management Progress just to remember what's exposed, the private Command Centre itself shows small, deliberately subordinate indicators — a "Management Visible" tag next to a Matter wherever it's browsed (the Client Workspace's Engagement → Matter → Workstream summary, §13.3, and the Projects/Matters register, §16.5's own §5) and a quiet "Management: Reference"/"Management: For Review" tag on a Document's own detail drawer (§13.6) when its `managementVisibility` is set. These reuse the same small `fn-chip` component already used elsewhere for tags (§13.2) precisely so they read as a minor administrative note, not a status a reader's eye is drawn to — private controls for the account holder, never the document/Matter's primary identity.
...
**Future secure sharing model (documented now, not built) — decided, 21 Sep 2026:** once a real recipient outside this session needs access, the preferred direction is:

1. **Authenticated management login — selected.** The boss gets a real account (most naturally through `platform-services/identity`, already the platform's own answer to "who is this and what may they see," §0) and the Management Progress route becomes a permission-gated view rather than a page anyone with a link can open. Confirmed as the preferred eventual model over a restricted or time-limited link — the account holder would rather provision a real account for a recurring reader than manage link secrecy/rotation. **Not built yet** — Management Progress remains preview-only inside this prototype (§16.5) until this is explicitly scoped as its own piece of work.
2. **Restricted, revocable read-only link** — a signed, unguessable URL tied to a specific viewer/session, revocable at any time, with no edit capability — kept on record as a fallback for a single recipient without provisioning a full account, provided the link itself is treated as a secret (not pasted into a public or persistent unencrypted channel), but not the preferred path.
3. **Time-limited secure link** — the same as (2) plus an expiry, best for a one-off "here's where things stand this week" send rather than a durable reference; also a fallback, not the preferred path.
