/**
 * Shared helper for Organisation/Employee Master integration tests that run
 * against a real Postgres instance (DATABASE_URL) — the SAME physical
 * database as platform-services/identity and platform-services/data-vault
 * (one shared schema). Reuses Identity's own migration runner and Postgres
 * connection provider by source import rather than duplicating them — see
 * src/composition/container.ts's header comment on dependency direction.
 * If DATABASE_URL is not set, tests using this helper skip with a clear
...
export async function withTestDb<T>(fn: (db: ReturnType<typeof createPgDatabaseProvider>) => Promise<T>): Promise<T> {
  ...
  try {
    // Full reset: this package's own tables plus every Identity table these
    // tests create fixtures in (users/roles/entity grants/audit/links) —
    // kept seeded org rows (groups, legal_entities) intact.
    await db.query(`
      TRUNCATE TABLE
