 ...
 * already-accepted SVEGIP session-cookie bridge to Organisation. Mirrors
...
import type { OrganisationContainer } from "../../src/composition/container.ts";
...
function fakeContainer(overrides: Partial<OrganisationContainer> = {}): OrganisationContainer {
  return {
    users: {
      findById: async () => null,
    ...
    assignments: undefined as never,
    orgStructure: undefined as never,
    svegipBridgeSecret: SECRET,
    trustedOrigins: new Set([TRUSTED_ORIGIN]),
    ...overrides,
  } as OrganisationContainer;
}
...
test("native bearer-token requests are never subject to the origin check", async () => {
  ...
  const container = fakeContainer({
    sessions: {
      ...
      listActiveSessions: async () => [],
      expireDueSessions: async () => 0,
    } as OrganisationContainer["sessions"],
    users: {
    ...
    } as OrganisationContainer["users"],
  });
