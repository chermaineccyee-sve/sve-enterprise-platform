/**
 * Shared helper for Data Vault integration tests that run against a real
 * Postgres instance (DATABASE_URL) — the SAME physical database as
 * platform-services/identity (one shared schema, not two). Reuses
 * Identity's own migration runner and Postgres connection provider by
 * source import rather than duplicating them — see
...
export async function withTestDb<T>(fn: (db: ReturnType<typeof createPgDatabaseProvider>) => Promise<T>): Promise<T> {
  ...
  try {
    // Full reset: Data Vault's own tables plus every Identity table these
    // tests create fixtures in (users/roles/entity grants/audit) — kept
